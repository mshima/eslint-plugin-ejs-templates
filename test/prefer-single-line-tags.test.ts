...
describe('autofix: prefer-single-line-tags', () => {
  test('keeps multiline tag without structural braces unchanged', () => {
    const input = '<%_\n  const x = 1;\n  const y = 2;\n_%>';
    expect(applyFix(input, { 'ejs-templates/prefer-single-line-tags': 'error' })).toMatchInlineSnapshot(`
      "<%_
        const x = 1;
        const y = 2;
      _%>"
  ...
  test('fix is idempotent', () => {
    ...
    expect(applyFix(fixed, { 'ejs-templates/prefer-single-line-tags': 'error' })).toMatchInlineSnapshot(`
      "<%_
        code;
      _%>"
    `);
  ...
  test('does not collapse code onto a // comment line', () => {
    ...
    expect(fixed).toMatchInlineSnapshot(`
      "<%_ // An embedded entity should not reference entities that embed it
         _%>
  ...
  test('splits content with brace boundaries (if/body/close each get own tag)', () => {
    const input = '<%_\n  if (x) {\n  doWork();\n_%>\n<%_  }\n_%>';
    const fixed = applyFix(input, { 'ejs-templates/prefer-single-line-tags': 'error' });
    expect(fixed).toMatchInlineSnapshot(`
      "<%_ if (x) { _%>
      <%_ doWork(); _%>
      <%_  }
  ...
  test('with slurp tags keeps contents in correct order', () => {
    ...
    expect(fixed).toContain('if (y) {');
    expect(fixed.indexOf('doWorkA();')).toBeLessThan(fixed.lastIndexOf('doWorkC();'));
  });
  ...
  test('with control block containing object literal (object literal braces never split)', () => {
    ...
    expect(fixed).toContain("beans.push({ foo: 'bar' });");
  });
...
describe('rule: ejs-templates/prefer-single-line-tags – multiple brace boundaries', () => {
  ...
  // The author's opening delimiter goes on the first tag only — repeating `<%_` would ask for
  // the whitespace before the tag to be slurped once per generated tag. Later tags open with
  // whatever pairs with the closing delimiter.
  test.each([
    ['plain', 'x<% if (a) { %>A<% if (b) { %>B<% } } %>;\n', 'x<% if (a) { %>A<% if (b) { %>B<% } %><% } %>;\n'],
    [
      'a slurping close',
      'x<% if (a) { %>A<% if (b) { %>B<% } } _%>;\n',
      'x<% if (a) { %>A<% if (b) { %>B<% } _%><%_ } _%>;\n',
    ],
    [
      'a slurping open',
      'x<%_ if (a) { _%>A<%_ if (b) { _%>B<%_ } } %>;\n',
      'x<%_ if (a) { _%>A<%_ if (b) { _%>B<%_ } %><% } %>;\n',
    ],
    [
      'slurping both sides',
      'x<%_ if (a) { _%>A<%_ if (b) { _%>B<%_ } } _%>;\n',
      'x<%_ if (a) { _%>A<%_ if (b) { _%>B<%_ } _%><%_ } _%>;\n',
    ],
  ])('splits an inline tag with %s without changing what surrounds it', (_label, template, expected) => {
